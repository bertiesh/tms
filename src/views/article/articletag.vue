        ...
        <el-dialog :title="isadd ? '添加' : '编辑'" :visible.sync="userform">
          ...
          >
            <el-form-item label="标签名" prop="tagName">
              <el-input v-model="rowdata.tagName"></el-input>
            </el-form-item>
...
<script>
import { getArticleTags,addArticleTag,deleteArticleTag} from "@/api/article/articletag";
...
this.$nextTick(() => {
    ...
    deleteRow(row) {
      this.$confirm("确定要删除吗？", this.$t("tip"), {
        ...
        .then(() => {
            deleteArticleTag([row.id]).then(
              (res) => {
                alertMessage(this, "success", "删除标签成功!");
                this.onLoad();
              },
              (error) => {
                alertMessage(this, "error", "删除标签出错!");
              }
            );
        })
        .catch(() => {});
    ...
    onLoad() {
      ...
      getArticleTags(dataobj).then((res) => {
